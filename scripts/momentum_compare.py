...
def run_momentum(monthly_returns, momentum, bench_monthly, top_n):
    rows = []
...
def main():
    ...
    momentum = monthly.pct_change(6).shift(1)
    bench_monthly = bench.resample("ME").last().pct_change().dropna()

    print("\nMomentum Top-N Comparison")
    print("-" * 40)
    for top_n in [3, 5, 10]:
        equity, bench_equity, latest = run_momentum(monthly_returns, momentum, bench_monthly, top_n)
        print(f"\nTop {top_n}")
        ...
        print(f"Nifty Final Value: ₹{bench_equity.iloc[-1]:,.2f}")
